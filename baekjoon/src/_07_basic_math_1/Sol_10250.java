...
class Sol_10250 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < testCase; i++) {
            String[] hotelInfo = br.readLine().split(" ");
            int H = Integer.parseInt(hotelInfo[0]);     // 층
            int W = Integer.parseInt(hotelInfo[1]);     // 방 수
            int N = Integer.parseInt(hotelInfo[2]);     // N번째 손님

            // 핵심 알고리즘 = 엘레베이터로 부터 가까운 방 선호 + 같은 거리면 낮은 층 선호
            // 층 선택 => N / H + 1
            // 호수 선택 => N % H + 1

            int X = N % H == 0 ? H : N % H;
            int Y = N % H == 0 ? N / H : (N / H) + 1;
