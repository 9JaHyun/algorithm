...
class Sol_1712 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int A = Integer.parseInt(info[0]);      // A = 고정 지출
        int B = Integer.parseInt(info[1]);      // B = 생산 단가
        int C = Integer.parseInt(info[2]);      // C = 노트북 가격

        int result = calcBEP(A, B, C);
        System.out.println(result);
    }

    private static int calcBEP(int a, int b, int c) {
//        1000 70 160
        if (b == c) {
            return -1;
        }
        int amount = (int) Math.round(a / (c - b)) + 1;
