package level1._1845_phoneMonster;

import java.util.HashSet;
import java.util.Set;

/**
 * @Question 당신은 폰켓몬을 잡기 위한 오랜 여행 끝에, 홍 박사님의 연구실에 도착했습니다.
...
class Solution {
    ...
    // 1. N/2마리 고르기 가능
    // 2. 겹치면 안된다. -> 중복을 허용하지 않는 Set이 좋을거 같음.
    private static int solution(int[] nums) {
        Set<Integer> pickSet = new HashSet<>();

        for (int phoneMonster : nums) {
            pickSet.add(phoneMonster);
        }

        return Math.min(pickSet.size(), (nums.length / 2));
    }
}
